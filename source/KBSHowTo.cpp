...
const wchar_t* const kHowToEN =
	...
	L"- Some matches move the view but are NOT selected, and keep the mark instead: overset text (there is nothing on the page to select), a locked one (InDesign offers no way to change locked content), one on a hidden layer (it draws nothing), a GREP match with no width (^, $ and the lookarounds match at a position rather than over characters, so there is nothing to highlight), and one whose text has been edited since the search.\n"
	L"- The up and down arrow keys walk the whole tree, opening the rows they pass through.\n"
	L"- Hide Previous Chapter: with it ON, the other document windows with nothing to save are closed as a jump lands. One that has been edited, or has never been saved at all, is left where it is.\n"
	L"\n"
	L"[Replacing (Change Checked)]\n"
	L"- Every row starts ticked: untick the ones you do not want and run Change Checked from the menu - only the ticked occurrences are replaced.\n"
	L"- Replaces with Track Changes on. After replacing, a change can be undone with Reject Change in Track Changes.\n"
	...
	L"- Minimizable Find/Change: puts a minimize box on InDesign's own Find/Change dialog, so it can be put out of the way without being closed. Windows only.\n"
	L"- These start OFF every time InDesign is launched. Save Panel Settings keeps them - and Hide Previous Chapter - for the next launch.\n"
	L"\n"
	L"[The Book panel (Remember Book Panel Placement)]\n"
	L"- ON: InDesign's own Book panel opens where it was, at the size it was, and collapsed to icons if it was, when it was last closed. That is written down as a book closes, and when InDesign quits.\n"
	L"- Docked, it goes back into the dock next to the same panels (the same tab group, or above or below the same group). Floating, it goes back to the same place and size.\n"
	L"- Ticking or unticking it is saved at once, and the Book panel's place is saved as it closes. Nothing else in the saved settings is touched by either.\n"
	L"\n"
	L"[Limits]\n"
...
const wchar_t* const kHowToJA =
	...
	L"・次の場合は、移動はしますが選択はせず、ジャンプの印が残ります：あふれ（オーバーセット）／ロックされている／非表示レイヤーにある／幅のない GREP の一致（^、$、先読みなどは文字ではなく位置に一致するので、選択する文字がありません）／検索後に編集されている。\n"
	...
	L"\n"
	L"【置換（Change Checked）】\n"
	L"・検索直後はすべての行にチェックが入っています。置換しない行のチェックを外し、メニューの Change Checked を実行すると、チェックした箇所だけが置換されます。\n"
	L"・変更履歴をオンにして置換します。置換後、変更履歴の変更を取り消しで戻す事が可能です。\n"
	...
	L"・Minimizable Find/Change: InDesign 本体の 検索と置換 ダイアログに最小化ボタンを付けます。閉じずにしまっておけます。Windows のみ。\n"
	L"・これらは InDesign を起動するたびに OFF から始まります。Save Panel Settings を実行しておくと、これらと Hide Previous Chapter が、次回の起動でも同じ状態で開きます。\n"
	L"\n"
	...
	L"・ドッキングしていたときは、同じパネルの隣（同じタブグループ、または同じグループの上か下）へ戻します。浮いていた（フロート）ときは、同じ位置と大きさへ戻します。\n"
	L"・チェックの ON/OFF はその場で保存され、位置と大きさはブックパネルを閉じたときに保存されます。保存済みのほかの設定には、どちらも触れません。\n"
	L"\n"
	L"【制限】\n"
