...
const wchar_t* const kHowToEN =
	...
	L"- Replacing searches the chapter again and gives the Nth match the Nth ticked row's replacement. It does not verify match by match that each one is still the one the search found, so an edit made in between can move a replacement onto something you did not tick.\n"
	L"- Against that: before anything is written, any chapter that has changed since the search is put to you, one at a time. Answering Cancel stops the whole run - nothing is written at all - and the results are cleared, because they no longer describe the text. Search again.\n"
	L"- \"Changed\" covers more than typing: when the search excluded hidden or locked content, showing/hiding or locking/unlocking a layer, hiding an object, or moving a frame onto such a layer also raises the prompt, because a re-search would visit a different set of matches.\n"
	L"\n"
...
const wchar_t* const kHowToJA =
	...
	L"・置換は章をもう一度検索し、N 番目の一致に N 番目のチェック行の置換を充てます。一致ごとに「検索したときと同じ箇所か」までは確認しないので、途中で編集していると、チェックしていない場所が置換されることがあります。\n"
	L"・その対策として、書き込む前に、検索後に変更のあった章を章ごとにお知らせします。キャンセルすると実行全体が中止され（1文字も書き込まれません）、結果一覧も消えます（その一覧はもう今のテキストを表していないため）。検索し直してください。\n"
	L"・「変更」は入力だけではありません。検索が非表示やロックを対象外にしていた場合は、レイヤーの表示/ロックの切り替え、オブジェクトの非表示、そうしたレイヤーへのフレーム移動もお知らせの対象です（再検索すると一致の並びが変わるため）。\n"
	L"\n"
