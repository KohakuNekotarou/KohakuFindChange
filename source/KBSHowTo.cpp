...
const wchar_t* const kHowToEN =
	...
	L"- ON: InDesign's own Book panel opens where it was, and at the size it was, when it was last closed. That place is written down as the Book panel closes, and when InDesign quits.\n"
	L"- Only while the Book panel floats. A docked one is left to its dock.\n"
...
const wchar_t* const kHowToJA =
	...
	L"・ON にすると、InDesign 本体のブックパネルが、前回閉じたときの位置と大きさで開きます。位置と大きさは、ブックパネルを閉じたときと InDesign を終了したときに記録します。\n"
	L"・ブックパネルが浮いている（フロート）ときだけ働きます。ドッキングしているときはドックに任せます。\n"
