...
const wchar_t* const kHowToEN =
	...
	L"- Against that: before anything is written, any chapter that has been edited since the search is put to you, one at a time. Answering Cancel stops the whole run - nothing is written at all - and the results are cleared, because they no longer describe the text. Search again.\n"
	L"\n"
...
const wchar_t* const kHowToJA =
	...
	L"・その対策として、書き込む前に、検索後に編集された章を章ごとにお知らせします。キャンセルすると実行全体が中止され（1文字も書き込まれません）、結果一覧も消えます（その一覧はもう今のテキストを表していないため）。検索し直してください。\n"
	L"\n"
